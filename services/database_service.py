...
def is_positive(row):
    ...
    if isinstance(row["notation"], str):
        ...
        if row["default_type"] == "boolean":
            ...
            return (
                row["value"] == row["default_type"]
                if notation_dict["cancel"] == "positive"
                else row["value"] != row["default_type"]
            )
    else:
        return None
...
def get_ght_results(offset=-1):
    ...
    ght = pd.read_sql(
        "SELECT * FROM ght WHERE ts BETWEEN '{ts1}' AND '{ts2}'".format(ts1=start_of_week, ts2=end_of_week), connection
    )
    ...
    if len(ght) <= 0:
        ...
        raise Exception(
            "No results found for week {kw}. Start of week: {ts1} /  end of week: {ts2}".format(
                kw=kw, ts1=start_of_week, ts2=end_of_week
            )
        )
    ght_questions = pd.read_sql("SELECT * FROM ght_questions_daily WHERE notation IS NOT NULL AND multiplier > 0",
                                connection)
    ...
    ght["text"] = ght.apply(
        lambda row: "{ts}: {message} ({multiplier}) -> {value} ".format(
            ts=row["timestamp"], message=row["message"], value=row["value"], multiplier=row["multiplier_x"]
        ),
        axis=1,
    )
    ...
    close_server_connection(connection)
    return (ght[["text"]], round((current_multiplier / sum_multiplier_per_week) * MAX_PER_WEEK * days_used, 2), kw)
...
def add_or_update_row_koreader_page_stat(row, conn, table_name="koreader_page_stat"):
    with conn.cursor() as cursor:
        cursor.execute(f"SELECT * FROM {table_name} WHERE page = %s and start_time = %s",
