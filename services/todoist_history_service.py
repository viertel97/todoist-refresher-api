...
def fetch_days_new(days=2):
    ...
    start = (
        ...
        .replace(tzinfo=None, hour=23, minute=59, second=59, microsecond=9999)
        .astimezone(tz=local_tz)
    )
    ...
    act_df = df_standardization(
        transform_act(
            get_todoist_activity(
                event_type="completed", limit=100, until=until_param, offset=0, parent_project_id=HABITS_PROJECT_ID
            )
        )
    )
    length = len(act_df)
