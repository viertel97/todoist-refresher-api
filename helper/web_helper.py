...
MASTER_KEY, REWORK_EVENTS_BIN, CATEGORIES_BIN, HABIT_LIST_BIN= get_secrets(
    ["jsonbin/masterkey", "jsonbin/Rework-Events-bin", "jsonbin/categories-bin", "jsonbin/habit_list-bin"])
...
def get_rework_data_from_web():
    ...
    response = requests.get(REWORK_EVENTS_URL, headers={'User-Agent': 'Mozilla/5.0', 'X-Master-Key': MASTER_KEY})
...
def get_categories_data_from_web():
    ...
    response = requests.get(CATEGORIES_URL, headers={'User-Agent': 'Mozilla/5.0', 'X-Master-Key': MASTER_KEY})
    return response.json()["record"]


def save_categories_data_to_web(data):
    logger.info("saving categories data to web")
    response = requests.put(CATEGORIES_URL.replace("/latest", ""), json=data, headers={'User-Agent': 'Mozilla/5.0', 'X-Master-Key': MASTER_KEY})
