...
MASTER_KEY, REWORK_EVENTS_BIN, CATEGORIES_BIN, HABIT_LIST_BIN, NOTION_IDS_BIN = get_secrets(
    [
        "jsonbin/masterkey",
...
def get_habits_from_web():
    ...
    response = requests.get(
        HABITS_URL, headers={"User-Agent": "Mozilla/5.0", "X-Master-Key": MASTER_KEY}
    )
    return response.json()["record"]
